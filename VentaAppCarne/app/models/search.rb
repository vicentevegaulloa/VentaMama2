class Search < ApplicationRecord
  ...
  def find_sales
    ...
    sales = sales.joins({:sale_products => :product}).group("sales.id").having("SUM(sale_products.cantidad*products.unit_price) >= ?", total_min) if total_min.present?
    sales = sales.joins({:sale_products => :product}).group("sales.id").having("SUM(sale_products.cantidad*products.unit_price) <= ?", total_max) if total_max.present?
    ...
    if order.present?
      if order == "fecha"
        ...
        sales = sales.joins({:sale_products => :product}).group("sales.id").order("SUM(sale_products.cantidad*products.unit_price)")
      end
    else
      sales = sales.order(:client_id)
    end
