class Client < ApplicationRecord
  ...
  def deuda
    ...
    deuda = self.sales.joins(:sale_states).joins("INNER JOIN (#{inner_query_dates}) AS group_states ON sales.id=group_states.sale_id").joins(:sale_products => :product).where("group_states.sale_id=sales.id AND group_states.max_state=sale_states.state_id AND group_states.max_state=2").sum("sale_products.cantidad*products.unit_price*sale_products.real_weight")
    deuda.round
  end
end
